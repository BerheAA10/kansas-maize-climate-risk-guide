...
Write-Host "SAFE19_FIX12 - validating source package" -ForegroundColor Cyan
& python ".\scripts\validate_deployment.py"
if ($LASTEXITCODE -ne 0) {
    throw "SAFE19_FIX12 validation failed."
}

Write-Host "SAFE19_FIX12 - creating a new public bundle" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    throw "SAFE19_FIX12 public-bundle export failed."
}
...
if (-not $PublicPath) {
    $Public = Get-ChildItem -LiteralPath $Parent `
        -Directory `
        -Filter "*SAFE19_FIX12_PUBLIC_BUNDLE_*" `
...
if (-not $PublicPath) {
    throw "Could not locate the SAFE19_FIX12 public bundle after export."
}
...
Write-Host ""
Write-Host "Opening SAFE19_FIX12 on http://localhost:8510" -ForegroundColor Green
